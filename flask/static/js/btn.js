$(document).ready(function() {
    ...
    $("#conn_hds").click(function() {
        $.ajax({
            ...
            "dataType": "json",
            "contentType": "application/json",
            "url": 'http://'+$("#hds_server").val()+'/dataservice/v1/list?from=jdbc:///&info=jdbc:phoenix:192.168.103.151:2181',
            "timeout": 30000,
            success: function(result) {
                // clear original options
    ...
    $("#hds_table_list").change(function() {
        ...
        if (tbl_sel_idx != 0) {
            ...
            $.ajax({
                ...
                "dataType": "json",
                "contentType": "application/json",
                "url": 'http://'+$("#hds_server").val()+'/dataservice/v1/list?from=jdbc:///&info=jdbc:phoenix:192.168.103.151:2181' + '&table=' + tbl_name,
                "timeout": 30000,
                success: function(result) {
                    // clear original options
